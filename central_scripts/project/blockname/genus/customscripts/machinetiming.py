...
#wiretext=(re.split('---+',open('wires','r').read())[1])
#wirel={}
#wiref={}
#for i in wiretext.strip().split('\n'):
#	w=i.split()
#	wirel[w[-1]]=w[-7]
#	wiref[w[-1]]=w[-2]
...
kk=open(csv_file,'w')
kk.write('beginpoint,endpoint,period,libsetup,checktype,required,arrival,slack,launchclk,captureclk,skew,fanout,netdelay,netcount,instcount,invdelay,bufdelay,combodelay,seqdelay,totaldelay,invcount,bufcount,combocount,seqcount,portcount,load,wirelength\n')
r=open('machine.rpt','r').read()
...
for i in paths:
	...
	for line in datapath.split('\n')[1:]:
		...
		if ' NET ' in line:
			load+=float(aa[-6])
			netdelay+=float(aa[7])
			#fanout1+=int(wiref[aa[5]])
			netn=aa[5]
			#wirelength+=celld[netn]
			#wirelength+=float(wirel[netn])
			netcount+=1
		if ((' INST ' in line)):
			instcount+=1
			fanout+=int((aa[-2]))
	...
	#print(fanout,fanout1)
	kk.write(f'{beginpoint},{endpoint},{otherclkarr},{libsetup},{check_type},{required},{arrival},{slack},{launchclk},{captureclk},{skew},{fanout},{round(netdelay,6)},{netcount},{instcount},{invdelay},{bufdelay},{combodelay},{seqdelay},{totaldelay},{invcount},{bufcount},{combocount},{seqcount},{portcount},{load},{wirelength}\n')
	#exit()
